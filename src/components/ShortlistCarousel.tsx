...
function CarouselSummary({ text, lang }: { text: string; lang: Lang }) {
  ...
  return (
    <p className="mt-3 max-w-2xl text-sm leading-relaxed line-clamp-2 text-white/60">
      {translated}
    </p>
  );
}
        ...
        <div className="relative overflow-hidden aspect-video sm:aspect-auto sm:h-[800px]" style={{ background: "#000" }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-12">
            <div className="sm:hidden">
              {video.brand && <span className="text-xs font-bold text-white/50">{video.brand}</span>}
              <h2 className="mt-1 text-lg font-black leading-tight text-white">{video.title}</h2>
            </div>
            ...
            <div className="hidden sm:block">
              <div className="mb-3 flex items-center gap-3">
                ...
                {video.tag && (
                  <span className="text-[11px] font-bold uppercase text-white/50">{video.tag}</span>
                )}
              ...
              <h2 className="text-3xl lg:text-4xl font-black leading-tight text-white">{video.title}</h2>
      ...
      {saved.length > 1 && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center opacity-0 transition-opacity group-hover:opacity-100 cursor-pointer bg-black/60 text-white/80 hover:bg-black/80"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6" /></svg>
          </button>
          <button
            onClick={(e) => { e.preventDefault(); next(); }}
            className="absolute right-24 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center opacity-0 transition-opacity group-hover:opacity-100 cursor-pointer bg-black/60 text-white/80 hover:bg-black/80"
          >
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1">
            {saved.map((_, i) => (
              <button
                ...
                style={{ width: i === current ? "20px" : "6px", background: i === current ? "var(--accent)" : "rgba(255,255,255,0.3)" }}
              />
            ))}
          </div>
        </>
