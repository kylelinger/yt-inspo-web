    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b" style={{ borderColor: "var(--nav-border)", background: "var(--nav-bg)", backdropFilter: "blur(20px)" }}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 sm:px-8 h-20">
        <a href="/" className="flex items-center gap-2 min-w-0">
          ...
          <span className="whitespace-nowrap text-[17px] font-black tracking-[-0.01em] text-[var(--text)]">Claw Pips</span>
        </a>

        <div className="flex items-center gap-4 sm:gap-6 text-[13px] sm:text-[14px] font-semibold uppercase tracking-[0.12em] text-[var(--text-secondary)]">
          {links.map((link) => (
          ...
          <button
            onClick={toggleLang}
            className="rounded border px-2.5 py-1 text-[11px] font-bold tracking-wider text-[var(--text-muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
          ...
          {isAdmin && (
            <a href="/admin" className="hidden sm:inline text-[var(--text-secondary)] transition-colors hover:text-[var(--accent)]" title="Admin">
